...
func_add_dns() {
  if [ $cloud_provider == "aws" ]; then
    export search_domain="fhlbny.net"
    export dns1="10.2.222.11"
    export dns2="10.2.222.12"
  fi
  ...
  #  docker-machine ssh ${basename}${i} "sudo echo 'search ${search_domain}' >> /etc/resolv.conf && \
...
if [ -z ${THIS_ZONE} ] || [ "${THIS_ZONE}" == "a" ]; then
  cat /dev/null > Docker-info.txt
fi
